...
func TestSignupAllowlist(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := svc.Register(context.Background(), tc.email, "weak")
			if tc.allowed {
...
func TestSignupAllowlistStore(t *testing.T) {
	...
	if err := svc.Register(context.Background(), "stranger@evil.com", "weak"); !errors.Is(err, ErrSignupNotAllowed) {
		t.Errorf("uninvited email with store wired: expected ErrSignupNotAllowed, got %v", err)
	}
	if err := svc.Register(context.Background(), "Invited@Corp.com", "weak"); !errors.Is(err, ErrWeakPassword) {
	...
	if err := svc.Register(context.Background(), "env@corp.com", "weak"); !errors.Is(err, ErrWeakPassword) {
